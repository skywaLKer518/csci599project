...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        #LK: for momentum
        self.model_update_W = theano.shared(
            value=numpy.zeros(
                (n_in, n_out),
                dtype=theano.config.floatX
            ),
        ...
        #LK: for momentum
        self.model_update_b = theano.shared(
            value=numpy.zeros(
                (n_out,),
                dtype=theano.config.floatX
            ),
        ...
        #LK: parameter for momentum
        self.model_update = [self.model_update_W, self.model_update_b]
...
def sgd_optimization_mnist(learning_rate=0.13, momentum=0, n_epochs=25,
...
#    ]
    ...
    updates1 = [
        ...
        for param_i, model_update_i, grad_i in zip(classifier.params, classifier.model_update,grads) 
    ]
    updates2 = [
        (model_update_i, model_update_i * momentum - learning_rate *grad_i)
        for model_update_i, grad_i in zip(classifier.model_update, grads)
